...
export class DatabaseStorage implements IStorage {
  ...
  async getEarningsForCreator(creatorId: number): Promise<EarningsStats> {
    // Optimization: Use SQL aggregations to perform calculations at the database level.
    // This reduces data transfer from O(N) to O(1) and eliminates O(N) in-memory processing.
